...
train_iter =  data.BucketIterator(dataset = train_data,batch_size=BATCH_SIZE,device=device,repeat=False,  sort_within_batch=True,shuffle=True)
...
test_iter =  data.BucketIterator(dataset = test_data,batch_size=BATCH_SIZE,device=device,repeat=False, sort_within_batch=True,shuffle=True)
...
class BiLstmCrf(nn.Module):
    def __init__(self, args):
        ...
        self.embedding = nn.Embedding(vocabulary_size, embedding_dimension,padding_idx=pad_idx)
...
optimizer = optim.SGD(model.parameters(), lr=0.007, weight_decay=1e-4,momentum=0.9)
...
for epoch in range(N_EPOCHS):
    ...
    for batch in tqdm(valid_iter):
        ...
        optimizer.step()
        # print(model.embedding.weight.data[UNK_IDX])
        # print('pad的值变了吗')
